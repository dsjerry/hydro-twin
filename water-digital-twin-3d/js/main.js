...
(() => {
  ...
  function init() {
    ...
    // 首次加载地图（异步加载 GeoJSON）
    const data = MockData.generateAll();
    ChartRenderer.renderGuangdongMap('chart-map', data);
    refreshAll();
    initAlertScroll();
    ...
    console.log('🌊 水利数字孪生平台（ECharts GL 3D版）已启动');
    console.log('📊 数据每30秒自动刷新，3D地图自动旋转，点击可暂停');
  }
